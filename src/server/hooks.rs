...
impl serverHooks {
    ...
    pub fn beforeInitialization (riff: MutexGuard<restfulAPI>, computation_id: Value, params: Value) {
        
        let party_count = &params["party_count"];
        // validate party_count
        if *party_count == Value::Null { // no party count given or saved.
            panic!("party count is not specified nor pre-saved");
        } else if party_count.as_u64().unwrap() < 1 { // Too small
            panic!("party count is less than 1");
        } else if riff.computationMaps.maxCount[computation_id.to_string()] != Value::Null && *party_count != riff.computationMaps.maxCount[computation_id.to_string()] {
        ...
        if *party_id != Value::Null { // party_id is given, check validity
            if *party_id != "s1" {
                if !party_id.is_number() || party_id.as_u64().unwrap() <= 0 || party_id.as_u64().unwrap() > party_count.as_u64().unwrap() {
            ...
            if riff.computationMaps.clientIds[computation_id.to_string()] != Value::Null && riff.computationMaps.clientIds[computation_id.to_string()].as_array().unwrap().len() == riff.computationMaps.maxCount[computation_id.to_string()] {
