...
pub fn put_in_mailbox(
    ...
    if computation_mailbox[to_id.to_string()] == Value::Null {
        computation_mailbox
            .as_object_mut()
            .unwrap()
            .insert(to_id.to_string(), json!([]));
    }

    // add message to mailbox, return pointer
    computation_mailbox[to_id.to_string()]
        .as_array_mut()
        .unwrap()
        .push(json!({
            "label": label,
    ...
    return riff.mail_box[computation_id.to_string()][to_id.to_string()]
        .as_array_mut()
        .unwrap()
        .len()
        - 1;
...
pub fn get_from_mailbox(
    ...
    if computation_mailbox[party_id.to_string()] == Value::Null {
        computation_mailbox
            .as_object_mut()
            .unwrap()
            .insert(party_id.to_string(), json!([]));
    }

    let mut result = Vec::new();
    let mut counter = 0;
    for item in computation_mailbox[party_id.to_string()]
        .as_array()
        .unwrap()
    {
        result.push(json!({
...
pub fn sliceMailbox(
    ...
    computation_id: Value,
    party_id: Value,
    mailbox_pointer: Value,
) {
    if riff.mail_box[computation_id.to_string()] != Value::Null
        && riff.mail_box[computation_id.to_string()][party_id.to_string()] != Value::Null
    {
        ...
        let sliced_mailbox = mailbox
            .as_array_mut()
            .unwrap()
            .split_off(number_pointer as usize + 1);
