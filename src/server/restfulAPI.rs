...
impl restfulAPI {
    ...
    async fn listen(
        ...
        match (req.method(), req.uri().path()) {
            ...
            (&Method::POST, "/poll") => {
                ...
                restful_instance.lock().unwrap().freeTag(
                ...
                let output = restful_instance.lock().unwrap().handleMessage(computation_id.clone(), from_id.clone(), msg["messages"].clone());
                if !output["success"].as_bool().unwrap() {
                ...
                let dumped = restful_instance
                    .lock()
                    .unwrap()
                    .dumpMailbox(computation_id, from_id);
                response
                    .as_object_mut()
                    .unwrap()
                    .insert(String::from("messages"), dumped["messages"].clone());
                if dumped["ack"] != Value::Null {
                    response
                        .as_object_mut()
                        .unwrap()
                        .insert(String::from("ack"), dumped["ack"].clone());
                }
    ...
    fn initializeParty(mut msg: Value, instance: Arc<Mutex<restfulAPI>>) -> Value {
        ...
        output
            .as_object_mut()
            .unwrap()
            .insert(String::from("party_id"), inner);
    ...
    pub fn initComputation(
        ...
        if self.computationMaps.clientIds[computation_id.to_string()] == Value::Null {
            ...
            self.computationMaps
                ...
                .as_object_mut()
                .unwrap()
                .insert(computation_id.to_string(), party_count.clone());
            self.computationMaps
                .freeParties
                .as_object_mut()
                .unwrap()
        ...
        if self.maps.tags[computation_id.to_string()][party_id.to_string()] == Value::Null {
            self.maps.tags[computation_id.to_string()]
                .as_object_mut()
                .unwrap()
                .insert(party_id.to_string(), json!(0));
        }
    }

    pub fn freeComputation(&mut self, computation_id: Value) {
        self.computationMaps.clientIds.as_object_mut().unwrap().remove(&computation_id.to_string());
        self.computationMaps.spareIds.remove(&computation_id.to_string());
        self.computationMaps.maxCount.as_object_mut().unwrap().remove(&computation_id.to_string());
        self.computationMaps.freeParties.as_object_mut().unwrap().remove(&computation_id.to_string());
        self.computationMaps.keys.as_object_mut().unwrap().remove(&computation_id.to_string());
        self.computationMaps.secretKeys.as_object_mut().unwrap().remove(&computation_id.to_string());
    ...
    pub fn dumpMailbox(&mut self, computation_id: Value, party_id: Value) -> Value {
        ...
        let x: u64 = 2;
        let LARGE = x.pow(32);
        let tag = (self.maps.tags[computation_id.to_string()][party_id.to_string()]
            .as_u64()
            .unwrap()
            + 1)
            % LARGE;
        self.maps.tags[computation_id.to_string()]
            .as_object_mut()
            .unwrap()
            .insert(party_id.to_string(), json!(tag));
        self.maps.pendingMessages[computation_id.to_string()]
            .as_object_mut()
            .unwrap()
            .insert(
                party_id.to_string(),
    ...
    pub fn freeTag(&mut self, computation_id: Value, party_id: Value, tag: Value) {
        if tag != Value::Null
            && self.maps.pendingMessages[computation_id.to_string()] != Value::Null
            && self.maps.pendingMessages[computation_id.to_string()][party_id.to_string()]
                != Value::Null
        {
            let pending =
                self.maps.pendingMessages[computation_id.to_string()][party_id.to_string()].clone();
            if tag == pending["tag"] {
                mailbox::sliceMailbox(
                    self,
                    computation_id.clone(),
                    party_id.clone(),
    ...
    pub fn handleMessage(&mut self, computation_id: Value, from_id: Value,mut messages: Value) -> Value {
